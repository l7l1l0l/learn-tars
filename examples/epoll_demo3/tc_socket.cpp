/***********************************************************************
* File Name: tc_socker.cpp
* Author: RogerLuo
* mail: 13369189710@163.com
* Created Time: Mon 28 Jan 2019 12:16:05 PM CST
***********************************************************************/
...
int TC_Socket::accept(TC_Socket& tcSock, struct sockaddr* pstSockAddr, socklen_t& iSockLen) {
    ...
    while ((fd = ::accept(_sock, pstSockAddr, &iSockLen)) < 0 && errno == EINTR);
...
int TC_Socket::connectNoThrow(const string& sServerAddr, uint16_t port) {
    ...
    if (sServerAddr.empty()) {
        cout << "[TC_Socket::connectNoTrhead] server address is empty!" << endl;
    }
